...
export function generateMinimalQueueWorker(
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    if (request.method === "POST") {
      ...
      console.log(\`[Producer] Enqueued message: \${messageId}\`);
    ...
    return new Response("Queue Demo\\n\\nPOST / to enqueue a message", {
  ...
  async queue(batch: MessageBatch<QueueMessage>, env: Env): Promise<void> {
    ...
    console.log(\`[Consumer] Processing batch \${batchId} with \${batch.messages.length} messages\`);

    for (const message of batch.messages) {
      console.log(\`[Consumer] Processing message: \${message.body.id} - "\${message.body.body}"\`);
    }
    ...
    batch.ackAll();
    console.log(\`[Consumer] Batch \${batchId} completed successfully\`);
  }
...
// Full queue demo with Durable Objects and Dashboard
...
export default {
  ...
  async queue(batch: MessageBatch<QueueMessage>, env: Env): Promise<void> {
    ...
    // Log that it has been batched and is being processed
